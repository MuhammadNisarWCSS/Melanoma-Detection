...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-ink-600/60 bg-[#fbf6f2]/85 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2" title={s.label}>
            ...
            <span className="hidden sm:block text-[12px] text-slate-500">
              {status === 'online' ? 'Ready' : status === 'checking' ? 'Connecting…' : 'Unavailable'}
            </span>
          </div>
